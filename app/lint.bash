...
mapfile -d '' source_files < <(find "$script_dir/src" "$script_dir/include" -type f \( -name '*.c' -o -name '*.cpp' \) -print0)

if [[ ${#source_files[@]} -eq 0 ]]; then
  echo "error: no source files found in ${script_dir}/src or ${script_dir}/include for clang-tidy" >&2
  exit 1
fi
